...
5. **Memory Tree Sync & Failure Distillation**: Auto-distill any turn errors or user corrections into invariant prevention rules (`zg learn add`). Update persistent decisions, gotchas, and goals in `.agents/memory/`.
...
- **Universal CLI**: `zg` (`zg mcp`, `zg learn`, `zg pack`, `zg test`, `zg doctor`, `zg memory`).
